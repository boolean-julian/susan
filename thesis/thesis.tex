...
\chapter{Der SUSAN Kantendetektor}
	...
	\section{Der Algorithmus}\label{sec:thealgorithm}
		...
		\subsection{Das SUSAN-Prinzip}\label{ssec:susan_principle}
			...
			\begin{figure}[H]
				...
				\begin{tikzpicture}[fill=black, text=white]
					...
					\normalsize
				\end{tikzpicture}
				\caption{Das SUSAN Prinzip (links Eingang, rechts Antwort, $t=1$, $3\times3$ Maske)}
			...
			\begin{figure}[H]\centering
				...
				\begin{tikzpicture}[fill=black, text=white]
					...
					\normalsize
				\end{tikzpicture}
				\caption{Das SUSAN Prinzip findet Ecken und hebt diese sogar in der Antwort hervor, $t=1$, $3\times3$ Maske}
			...
			\begin{figure}[H]\centering
				...
				\begin{tikzpicture}[fill=black, text=white]
					...
					\normalsize
				\end{tikzpicture}
				\caption{Randbedingung mit den Vorzeichen $\sigma_i$. Insgesamt ist die Antwort stärker}
				\label{fig:noborder}
			\end{figure}
		...
		\subsection{Non-Maximum-Suppression}\label{ssec:nonmax}
			...
			Die dort vorgestellte Prozedur kategorisiert jedes Pixel nach der generellen Richtung jeder Kante mit $\frac{pi}{8}$ Genauigkeit. Senkrecht zu dieser Richtung wird dann nur das Maximum der direkten Nachbarschaft erhalten, der Rest wird aus dem Kantenbild entfernt (also $=0$ gesetzt).
