	...
	\section{Der Algorithmus}\label{sec:thealgorithm}
		...
		\subsection{Das SUSAN-Prinzip}\label{ssec:susan_principle}
			...
			\begin{figure}[H]\centering
				\includegraphics[width=.45\textwidth]{../examples/lena/lena.png}\quad
				\includegraphics[width=.45\textwidth]{../examples/lena/15_out_raw.png}
				\caption{Ein Testdurchlauf mit $t=15$ und der $7\times 7$ Maske}
			\end{figure}


		\subsection{Non-Maximum-Suppression}\label{ssec:nonmax}
			Das oben genannte Prinzip findet Kanten innerhalb von Bildern, aber die Lokalisation der Kanten könnte besser sein. Im Bereich der eigentlichen Kante stellen wir fest, dass die Antwort $A$ ungleich $0$ ist. Um die Kante genauer zu lokalisieren, verwenden wir das Prinzip der Non-Maximum-Suppression, bei der nur die maximale Antwort entlang einer Kante erhalten bleibt.
			...
			Wir werden zunächst jede Antwort in eine der zwei Fälle einsortieren, danach können wir die Richtung bestimmen:
			...
			\begin{figure}[H]\centering
				\begin{tikzpicture}[fill=black, text=white]
					...
					\normalsize
				\end{tikzpicture}
				\caption{Eingangsbild wie in Abbildung [x]. Durch die Non-Maximum-Suppression wurde die Kante genau lokalisiert}
			\end{figure}
			...
			Die Non-Maximum-Suppression unterdrückt in unserem Beispiel tatsächlich alle diejenigen Pixel, die keine lokalen Maxima in der Richtung senkrecht zur Kante sind. Auch für größere Bilder erzielt die Non-Maximum-Suppression den erwünschten Effekt, siehe Abbildung [x].
			...
			\begin{figure}[H]\centering
				...
				\caption{Wie Abbildung [x]. Links das Bild mit den Kantenrichtungen (Skala wie in Abbildung [x]). In der Mitte das Bild nach dem prinzipiellen SUSAN-Schritt. Das Bild rechts ist nach der Non-Maximum-Suppression.}
		...
		\subsection{Ausdünnen}
			...
			Im Testbild der Abbildung [x] erkennt man die Funktionsweise in allen vier oben genannten Fällen.

			\begin{figure}[H]\centering
			\includegraphics[width=0.3\textwidth]{./assets/thinout/out_nonmax_supp.png}\quad
			\includegraphics[width=0.3\textwidth]{./assets/thinout/out_thinned.png}
			\caption{Links: Kantenbild. Rechts: Kantenbild nach dem Ausdünnen}
			\end{figure}
